...
func (s *Service) isSecurityGroupToDelete(resource *AWSResource) bool {
	if !s.isMatchingResource(resource, strings.ToLower(v2ec2.ServiceID), "security-group") {
		return false
	}
...
func (s *Service) deleteSecurityGroup(ctx context.Context, securityGroupID string) error {
	input := v2ec2.DeleteSecurityGroupInput{
...
func (s *Service) getProviderOwnedSecurityGroups(ctx context.Context) ([]*AWSResource, error) {
	input := &v2ec2.DescribeSecurityGroupsInput{
		Filters: []v2ec2types.Filter{
			*filter.EC2.ProviderOwned(s.scope.KubernetesClusterName()),
		},
	}

	var resources []*AWSResource
	paginator := v2ec2.NewDescribeSecurityGroupsPaginator(s.ec2Client, input)
...
// 	var resources []*AWSResource
// 	err := s.ec2Client.DeleteSecurityGroup(context.TODO(), input, func(out *v2ec2.DescribeSecurityGroupsOutput, last bool) bool {
